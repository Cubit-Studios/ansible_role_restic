...
| `monitoring_call_failure`  |           no          | A command that will be called if the backup is *not successful*. Use the full command, you need to run. Example: `curl https://monitoring.example.com/api/push/E9Wzm4lJ2O?status=up&msg=OK&ping=` |
